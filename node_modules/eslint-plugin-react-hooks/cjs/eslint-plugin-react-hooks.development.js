      ...
      switch (callback.type) {
        ...
        case 'Identifier':
          ...
          if (declaredDependenciesNode.elements && declaredDependenciesNode.elements.some(function (el) {
            return el.type === 'Identifier' && el.name === callback.name;
          })) {
      ...
      reportProblem({
        ...
        suggest: [{
          ...
          fix: function (fixer) {
            ...
            return fixer.replaceText(declaredDependenciesNode, "[" + suggestedDeps.join(', ') + "]");
          }
        }]
      });
    }
...
function getDependency(node) {
  if ((node.parent.type === 'MemberExpression' || node.parent.type === 'OptionalMemberExpression') && node.parent.object === node && node.parent.property.name !== 'current' && !node.parent.computed && !(node.parent.parent != null && node.parent.parent.type === 'CallExpression' && node.parent.parent.callee === node.parent)) {
    return getDependency(node.parent);
  } else if (node.type === 'MemberExpression' && node.parent && node.parent.type === 'AssignmentExpression') {
    return node.object;
  } else {
    return node;
  }
...
function toPropertyAccessString(node) {
  if (node.type === 'Identifier') {
    ...
    var _object = toPropertyAccessString(node.object);

    var _property = toPropertyAccessString(node.property);
